...
__global__
void render_phong_light_kernel(image<rgb8_t> im, image<float3> vm, image<float4> nm, intrinsics K, float3 light, float3 view)
{
...
void render_phong_light(image<rgb8_t>* im, const image<float3>* vm, const image<float4>* nm, intrinsics K, float3 light, float3 view)
{
    ...
    render_phong_light_kernel<<<grid_size, block_size>>>(im->gpu(), vm->gpu(), nm->gpu(), K, light, view);
}
